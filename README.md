...
- 本文未定義でも頻出する専門語（定義が無いため実際には「意味」を表示できない）は、視覚的に区別した下線でマークし、クリックで「知っている」として今後表示しないようにできる
...
3. **読書ビュー**: 単一カラムの読みやすいレイアウトで本文が表示されます。
   - アクセントカラーの下線付き語句をクリックすると、定義がポップアップ表示されます
   - 薄い色の下線付き語句（本文未定義の頻出語）は定義を持たないため、クリックすると「知っている」ボタンのみ表示されます（今後表示されなくなります）
